...
def calculate_stats(timestamp, duration='day'):
  ...
  if duration == 'day':
    drawing_qry = model.Drawing.query()
    drawing_qry = drawing_qry.filter(model.Drawing.created >= start)
    drawing_qry = drawing_qry.filter(model.Drawing.created < finish)
    stats_db.drawings = drawing_qry.count()
  else:
    ...
    for s_db in stats_dbs:
      stats_db.drawings += s_db.drawings
